...
│   │   ├── io.hpp               # Link between this STM32G4 and our inputs/outputs (like motor currents and PWM settings).
...
After generating the CubeMX configuration code we need to make the following changes:
    ...
    6. Copy the `Code` folder over.
    7. Add our sources and include paths to `CMakeLists.txt`:
        `
        # All cpp sources under the "Code/Src" directory
        file(GLOB_RECURSE USER_SOURCES "Code/Src/*.c" "Code/Src/*.cpp")
    ...
    Fix the linker warning by adding `(READONLY)` to the following lines, also inside `SECTIONS`:
        `
          ...
          .fini_array (READONLY) :
        `
    10. Attach the GPIO outputs to the proper routines (TODO: need a file where the pinout is defined all together).
    11. To properly setup VSCode we must copy over these files:
...
TODO: links to control interface
